...
export default function DailyOperationalHours({ config, setConfig }: ConfigProps) {
  return (
      ...
      <div className="flex flex-col xl:flex-row lg:flex-col sm:flex-row gap-4 w-full">
        <div className="bg-light-purple dark:bg-card border border-transparent p-4 rounded-xl flex-1 flex justify-between items-center group focus-within:border-dark-purple focus-within:ring-1 focus-within:ring-dark-purple transition-colors">
          <div>
            ...
            <input type="time" value={config.startTime} onChange={(e) => setConfig({...config, startTime: e.target.value})} className="bg-transparent text-2xl font-bold outline-none text-light-main dark:text-main scheme-light dark:scheme-dark" />
          </div>
        ...
        <div className="bg-light-purple dark:bg-card border border-transparent p-4 rounded-xl flex-1 flex justify-between items-center group focus-within:border-dark-purple focus-within:ring-1 focus-within:ring-dark-purple transition-colors">
          <div>
            ...
            <input type="time" value={config.endTime} onChange={(e) => setConfig({...config, endTime: e.target.value})} className="bg-transparent text-2xl font-bold outline-none text-light-main dark:text-main scheme-light dark:scheme-dark" />
          </div>
